            ...
            <div class="bg-white rounded-xl shadow-2xl overflow-hidden">
                ...
                <div class="bg-indigo-600 px-6 py-8 text-center">
                    <div
                        ...
                        <img src="https://ui-avatars.com/api/?name={{ urlencode($user->nama) }}&background=random&color=fff&size=128"
                            alt="{{ $user->nama }}" class="h-full w-full object-cover">
                    </div>
                    <h1 class="text-2xl font-bold text-white">{{ $user->nama }}</h1>
                    <p class="text-indigo-200 mt-1">{{ $user->prodi->nama_prodi ?? 'Belum memiliki program studi' }}</p>
                    <span class="inline-block mt-2 px-3 py-1 rounded-full text-xs font-semibold 
                        {{ $user->status === 'Aktif' ? 'bg-green-100 text-green-800' : 
                           ($user->status === 'Cuti' ? 'bg-blue-100 text-blue-800' : 
                           ($user->status === 'Lulus' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800')) }}">
                        {{ $user->status }}
                    </span>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="text-sm font-semibold text-gray-900">{{ $user->no_tlp }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="ml-3">
                                ...
                                <p class="text-sm font-semibold text-gray-900">{{ $user->alamat }}</p>
                            </div>
                        </div>
                    </div>
